	...
	class Calendar{
		...
		constructor(elem, opts){
			...
			this._dayClicked = function(e){
				var evtids = e.currentTarget.getAttribute('data-events').split(",");
				var evts = [];
				for(var i=0; i<evtids.length; i++) 
					if(this.events[evtids[i]]!==undefined) 
						evts.push(this.events[evtids[i]]);
				var date = new Date(this.year, this.month, e.currentTarget.getAttribute('data-day'));
		...
		async drawCalendar(){ 
			...
			this.elem.insertAdjacentHTML('beforeend', "<div class='cjs-weekRow cjs-calHeader'><div class='cjs-lastLink'>&vltri;</div><div class='cjs-nextLink'>&vrtri;</div><div class='cjs-moTitle'>"+monthName+" "+year+"</div></div>");
			// Draw day labels
